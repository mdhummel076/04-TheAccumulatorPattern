...
#
#              for k in range(r - s): This doesn't give the right values for k, it starts at 0 instead of r should be range(r,s+1)
#                  count = 0    This shouldnt be inside the for loop or it will reset to 0 every time
#                  if math.cos(r) > 0:  This should be the cosine of k, r never changes.
#                      count = 1    This doesn't increment count, it just sets it to one. should be count = count + 1
#                  return count This also should be outside the loop so that it only returns after everything has been counted
#
#     f. The code in the "graphics accumulation" example below includes:
#            for _ in range(n):
#        What does the   _   (underscore) mean?
#
#               It is a counting variable to represent which iteration of the loop you are on
#
...
#              for k in range(n):
#                  center = rg.Point(x + (k * diameter), y)
#                  circle = rg.Circle(point, radius)        This one is wrong because it adds the diameter to the position twice
#                  circle.attach_to(window)
#                  x = x + (2 * radius)
#
###############################################################################
